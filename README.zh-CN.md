...
```bash
# Claude Code
npx get-shit-done-cn --claude --global   # 安装到 ~/.claude/
npx get-shit-done-cn --claude --local    # 安装到 ./.claude/

# OpenCode（开源，可用免费模型）
npx get-shit-done-cn --opencode --global # 安装到 ~/.config/opencode/

# Gemini CLI
npx get-shit-done-cn --gemini --global   # 安装到 ~/.gemini/

# Codex（以 skills 为主）
npx get-shit-done-cn --codex --global    # 安装到 ~/.codex/
npx get-shit-done-cn --codex --local     # 安装到 ./.codex/

# 所有运行时
npx get-shit-done-cn --all --global      # 安装到所有目录
```
...
**命令行为不符合预期？**
- 运行 `/gsd:help` 确认安装成功
- 重新执行 `npx get-shit-done-cn` 进行重装
...
OpenCode、Gemini CLI 和 Codex 现在都已经通过 `npx get-shit-done-cn` 获得原生支持。
