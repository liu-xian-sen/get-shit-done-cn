...
function cmdWorkstreamList(cwd, raw) {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      status = stateExtractField(stateContent, 'Status') || stateExtractField(stateContent, '状态') || 'unknown';
      currentPhase = stateExtractField(stateContent, 'Current Phase') || stateExtractField(stateContent, '当前阶段');
    } catch {}
...
function cmdWorkstreamStatus(cwd, name, raw) {
  ...
  try {
    ...
    stateInfo = {
      status: stateExtractField(stateContent, 'Status') || stateExtractField(stateContent, '状态') || 'unknown',
      current_phase: stateExtractField(stateContent, 'Current Phase') || stateExtractField(stateContent, '当前阶段'),
      last_activity: stateExtractField(stateContent, 'Last Activity') || stateExtractField(stateContent, '最后活动'),
    };
  } catch {}
...
function cmdWorkstreamProgress(cwd, raw) {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      const phaseMatches = roadmapContent.match(/^###?\s+(?:Phase|阶段)\s+\d/gm);
...
function getOtherActiveWorkstreams(cwd, excludeWs) {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      status = stateExtractField(content, 'Status') || stateExtractField(content, '状态') || 'unknown';
      currentPhase = stateExtractField(content, 'Current Phase') || stateExtractField(content, '当前阶段');
    } catch {}

    if (status.toLowerCase().includes('milestone complete') ||
        status.toLowerCase().includes('里程碑完成') ||
