...
function buildContextMd(slice, phasePrefix) {
  const lines = [
    `# 阶段 ${phasePrefix} 上下文`,
    '',
    `从 GSD-2 切片 ${slice.id}：${slice.title} 迁移`,
  ];
...
function buildRoadmapMd(milestones, phaseMap) {
  ...
  for (const milestone of milestones) {
    ...
    for (const { slice, phaseNum } of mPhases) {
      ...
      lines.push(`- [${check}] **阶段 ${prefix}：${slug}** — ${slice.title}`);
    }
    lines.push('');
  }
...
function buildStateMd(phaseMap) {
  ...
  const status = currentEntry ? '准备规划' : '所有阶段已完成';
  ...
  return [
    ...
    '',
    `**当前焦点：** 阶段 ${currentPhaseNum}（${currentSlug}）`,
    '',
    '## 当前位置',
    '',
    `阶段：${currentPhaseNum} / ${zeroPad(totalPhases)}（${currentSlug}）`,
    `状态：${status}`,
    `最后活动：${today} — 从 GSD-2 迁移`,
    '',
    `进度：${bar} ${pct}%`,
    '',
    '## 累积上下文',
    ...
    '### 决策',
    '',
    '从 GSD-2 迁移。请查看 PROJECT.md 了解关键决策。',
    '',
    '### 阻塞/关注',
    '',
    '无。',
